...
class Trace(object):
    ...
    def allpoints(self,stops=False):
        ...
        """
        if stops:
            l=(self.iter_length(att='UTM',stops=stops)+self.iter_length(att='UTM',stops=False),2)
        else:
            l=(self.iter_length(att='UTM',stops=stops),2)
    ...
    def rgyr(self,add=True,stops=True):
        ...
        """
        a=(st.gyrad(self.allpoints(stops=stops))[-2],st.gyrad(self.allpoints(stops=stops))[-1])
...
class Flight(object):
    ...
    def rgyr(self,add=True):
        ...
        """
        a=(st.gyrad(self.UTM)[-2],st.gyrad(self.UTM)[-1])
...
# I am here 
    ...
    def __str__(self):
        return "## Stoping point  ##: \n N_points=%i \t Position=(%f,%f) \n Time=%f " % (self.N_points,self.mean_point_UTM[0],self.mean_point_UTM[1],self.mean_point_UTM[-1])
